package ControllerSidebar;

import java.io.IOException;

import javax.servlet.RequestDispatcher;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import BEAN.DMSanPham;
import DAO.DMSanPhamDAO;

import  java.util.*;

@WebServlet("/DanhMucSanPham")
public class DanhMucSanPham extends HttpServlet 
{
	private static final long serialVersionUID = 1L;
       
    public DanhMucSanPham() 
    {
        super();
    }

	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
			throws ServletException, IOException 
	{
		try 
		{
			List<DMSanPham> list_dmsp = DMSanPhamDAO.LayToanBoDMSanPham(request);
			request.setAttribute("list_dmsp", list_dmsp);
		} 
		catch (Exception e) 
		{
			
		}
		
		RequestDispatcher rd = request.getRequestDispatcher("/View/Admin/Sidebar/DanhMucSanPham.jsp");
		rd.forward(request, response);
	}

	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
			throws ServletException, IOException 
	{
		doGet(request, response);
	}

}
